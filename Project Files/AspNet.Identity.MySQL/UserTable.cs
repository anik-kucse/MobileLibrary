...
namespace AspNet.Identity.MySQL
{
        ...
        where TUser :IdentityUser
    {
        ...
        {
            string commandText = @"Insert into Users (father_name, mother_name, current_address, parmanent_address, date_of_birth, education, blood_group, 
                UserName, firstName, lastName, Id, PasswordHash, SecurityStamp,Email,EmailConfirmed,PhoneNumber,PhoneNumberConfirmed, AccessFailedCount,LockoutEnabled,LockoutEndDateUtc,TwoFactorEnabled,
                Sex)
                values (@faname, @maname, @cadd, @padd, @birth, @education, @blood,@name, @fn, @ln, @id, @pwdHash, @SecStamp,@email,@emailconfirmed,@phonenumber,
                @phonenumberconfirmed,@accesscount,@lockoutenabled, @lockoutenddate,@twofactorenabled, @sex)";
            Dictionary<string, object> parameters = new Dictionary<string, object>();
            parameters.Add("@faname", user.FatherName);
            parameters.Add("@maname", user.MotherName);
            parameters.Add("@cadd", user.CurrentAddress);
            parameters.Add("@padd", user.ParmanetAddress);
            parameters.Add("@birth", user.BirthDate);
            parameters.Add("@education", user.Education);
            parameters.Add("@blood", user.BloodGroup);
